...
## Session — notif read policy, strip color-match, mobile-landing deploy note

- **Notifications root cause (rows DO exist, confirmed via Supabase):** the restrictive
  `velo_notif_select using (user_id = auth.uid())` from the earlier migration filters them out.
  Fix = `SUPABASE_FIX_notifications_read.sql` → public-read SELECT (client already filters by
  user_id), matching trade_history. Run it to unblock the bell.
- **Market strip now color-matches other panels.** It was missing `background: var(--glass-bg)`
  on the wrapper (relied on cells + hairline gaps → greyer material, obvious in light mode). Now
  one glass panel identical to Trending, with `borderLeft` dividers (skip first) + theme-safe hover.
- **Mobile landing fix is in `index.html` (line ~1331) but NOT deployed** — velotrading.live is a
  SEPARATE Vercel project from app.velotrading.live. Must deploy the landing repo for the
  flow-card stacking fix to appear.

## Session — route fallback, faucet deposit, landing live-patch, catch-all

- **Landing mobile fix CONFIRMED not deployed** (curl of live velotrading.live showed old
  `flow-card { flex-direction: row }`). Patched the live-fetched file directly + re-verified render
  at 375px (art now stacks above text). The fix is correct; the LANDING project must be redeployed.
- **Unknown routes now redirect to main.** App `navigateFromPath` falls back to DASHBOARD (or TRADE
  if logged out) + cleans the URL. Landing `vercel.json` got a catch-all rewrite to `/index.html`
  (excludes blog/docs/assets/api/sitemap/robots/favicon/og).
- **Faucet/welcome deposit now records without a txHash** (was gated on `txHash && amount>0`; off-chain
  credits were skipped → missing from Recent Activity). Now records on `amount>0` with a synthetic ref.
- **Reminders (already in build/DB, deploy-dependent):** market strip uses `var(--glass-bg)` to match
  panels; notifications need `SUPABASE_FIX_notifications_read.sql` run (DB-only, public-read SELECT).
