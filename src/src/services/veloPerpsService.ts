...
export async function openPosition(
  ...
  // ── Warm the Pyth on-chain cache BEFORE opening ───────────────────────────
  // Base Sepolia testnet: the Pyth contract's updatePriceFeeds() silently no-ops
  // when the incoming publishTime <= cached publishTime. If the cache is stale,
  // the contract reads near-zero price and stores it as entryPrice_E18.
  // Fix: push a direct updatePriceFeeds tx to the Pyth contract first, which
  // guarantees the cache is fresh for the immediately-following openPosition tx.
  // The Pyth fee for a direct push is the same getUpdateFee amount.
  try {
    const PYTH_UPDATE_ABI = [{
      type: 'function', name: 'updatePriceFeeds', stateMutability: 'payable',
      inputs: [{ name: 'updateData', type: 'bytes[]' }], outputs: [],
    }] as const;
    const warmFee = await getExactPythFee(publicClient, updateData);
    const warmHash = await walletClient.writeContract({
      address: PYTH_CONTRACT_ADDRESS,
      abi: PYTH_UPDATE_ABI,
    ...
    await publicClient.waitForTransactionReceipt({ hash: warmHash });
    console.debug('[velo] Pyth cache warmed ✓ — fetching fresh update data');
    // Fetch fresh updateData so the openPosition tx also carries a valid blob
    const fresh = await fetchPriceUpdate([feedId]);
    updateData = fresh.updateData;
    parsedPrice = fresh.parsedPrice;
  } catch (warmErr: any) {
    // Non-fatal: log and continue — openPosition may still succeed if cache is fresh enough
    console.warn('[velo] Pyth warm tx failed (non-fatal):', warmErr?.shortMessage || warmErr?.message);
  }
