...
export const OrderDetailsModal = ({
  ...
  const headerPair = payload.kind === 'TRANSACTION' ? (payload.item.type === 'DEPOSIT' ? 'Deposit' : 'Withdrawal') : payload.item.pair;
  const headerSide = payload.kind === 'TRANSACTION' ? (payload.item.type === 'DEPOSIT' ? 'IN' : 'OUT') : payload.item.side;
  const sideUp = payload.kind === 'TRANSACTION' ? payload.item.type === 'DEPOSIT' : payload.item.side === 'LONG';
  ...
  if (payload.kind === 'TRANSACTION') {
    ...
    hero = (
      <div style={{ padding: heroPadding, borderBottom: '1px solid var(--hairline-strong)', textAlign: 'center', background: isDeposit ? 'rgba(34,197,94,0.06)' : 'rgba(239,68,68,0.06)' }}>
        <div style={{ ...S.label, fontSize: 10, marginBottom: 4, color: 'var(--fg-muted)' }}>{isDeposit ? 'Amount Deposited' : 'Amount Withdrawn'}</div>
        <div style={{ ...S.mono, fontSize: isSmall ? 26 : 30, fontWeight: 700, color: isDeposit ? 'var(--pnl-up)' : 'var(--pnl-down)' }}>
    ...
    body = (
      <>
        <R label="Type"      value={tx.type}                  tip={isDeposit ? "Funds added to your trading account." : "Funds withdrawn from your trading account."} />
        <R label="Amount"    value={`$${formatMoney(tx.amount)}`} valueColor={isDeposit ? 'var(--pnl-up)' : 'var(--pnl-down)'} tip="The amount transferred." />
        ...
        {tx.onChain && tx.txHash && (() => {
          ...
          // any vault interactions. Real deposits are 0x tx hashes — link to
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0, flexWrap: 'wrap' as const }}>
            ...
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, fontWeight: 700, padding: '3px 7px', borderRadius: 6, background: 'var(--chip-bg)', color: 'var(--fg-muted)', border: '1px solid var(--hairline-strong)' }}>{payload.kind === 'HISTORY' ? 'CLOSED' : payload.kind === 'POSITION' ? 'OPEN' : payload.kind === 'TRANSACTION' ? (payload.item.type === 'DEPOSIT' ? 'DEPOSIT' : 'WITHDRAW') : 'PENDING'}</span>
            {copyId && (
