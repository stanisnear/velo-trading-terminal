...
export const VeloManagePositionModal: React.FC<Props> = ({ isOpen, onClose, position, currentPrice, actions }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      setTab('ADD'); setBusy(false); setError(''); setLastTx(null);
      setAddAmount(''); setReduceAmount(''); setClosePct(50);
        ...
        <div style={{ padding: 18 }}>
          ...
          {tab === 'PARTIAL' && (
            <>
              ...
              <div style={{ ...S.label, marginBottom: 6 }}>Close {closePct}%</div>
              ...
              <Submit busy={busy} disabled={!v2Available} onClick={() => handle('PARTIAL')} icon={<Scissors size={12} />} label={`Close ${closePct}%`} />
            </>
          )}
          {tab === 'TRIGGERS' && (
            <>
...
const Submit: React.FC<{ busy: boolean; disabled: boolean; onClick: () => void; icon: React.ReactNode; label: string }> = ({ busy, disabled, onClick, icon, label }) => (
