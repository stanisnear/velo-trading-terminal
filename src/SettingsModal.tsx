        ...
        <div onClick={e => e.stopPropagation()} style={{ position:'relative', width:'100%', maxWidth:500, background:'var(--modal-bg, rgba(14,15,22,0.97))', borderRadius:28, border:'1px solid var(--hairline-strong)', boxShadow:'0 0 0 1px oklch(0.55 0.24 295 / 0.1), 0 40px 100px -20px rgba(0,0,0,0.65), 0 1px 0 oklch(1 0 0 / 0.07) inset', backdropFilter:'blur(48px) saturate(1.6)', WebkitBackdropFilter:'blur(48px) saturate(1.6)', overflow:'hidden', animation:'sm-card 0.36s cubic-bezier(0.22,1,0.36,1) both', color:'var(--fg)' }}>
          ...
          <div style={{ padding:'0 20px 26px', display:'flex', flexDirection:'column', gap:10, maxHeight:'68vh', overflowY:'auto', position:'relative', zIndex:1 }}>
            ...
            {burner ? (<>
              ...
              <div style={{ display:'grid', gridTemplateColumns:`repeat(${[!!onOpenSend, onOpenBridge, onOpenUsername].filter(Boolean).length}, 1fr)`, gap:8 }}>
                {/* Send is ALWAYS rendered when the host wires it — disabling
                    (rather than hiding) on zero balance matches Binance /
                    Hyperliquid UX. Users always see the action exists; the
                    disabled state + tooltip tells them why it's currently
                    unavailable. Hiding it created confusion across browser
                    profiles where one account had balance and the other
                    didn't, making it look like Velo was broken. */}
                {onOpenSend && (
                  <ActBtn
                    onClick={() => { if (veloUsdc > 0) onOpenSend(); }}
                    icon={<Send size={13}/>}
                    label="Send"
                    accent
                    disabled={veloUsdc <= 0}
                    title={veloUsdc <= 0 ? 'Deposit mUSDC to your trading wallet first' : undefined}
                  />
                )}
...
const ActBtn: React.FC<{ onClick:()=>void; icon:React.ReactNode; label:string; accent?:boolean; disabled?:boolean; title?:string }> = ({ onClick, icon, label, accent, disabled, title }) => (
  <button onClick={onClick} disabled={disabled} title={title} style={{ padding:'12px 8px', borderRadius:12, border:accent && !disabled ?'none':'1px solid var(--hairline-strong)', background: disabled ? 'var(--chip-bg)' : accent?'linear-gradient(135deg,oklch(0.45 0.26 295),oklch(0.65 0.22 268))':'var(--chip-bg)', color: disabled ? 'var(--fg-subtle)' : accent?'#fff':'var(--fg)', fontFamily:'var(--font-mono)', fontSize:10, fontWeight:700, letterSpacing:'0.06em', textTransform:'uppercase' as const, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.55 : 1, display:'flex', alignItems:'center', justifyContent:'center', gap:5, boxShadow: !disabled && accent ?'0 4px 14px -4px oklch(0.55 0.24 295 / 0.4)':'none', transition:'opacity 0.14s' }}
    onMouseEnter={e=>{ if (!disabled) e.currentTarget.style.opacity='0.85'; }} onMouseLeave={e=>{ if (!disabled) e.currentTarget.style.opacity='1'; }}>
    {icon} {label}
  </button>
);
